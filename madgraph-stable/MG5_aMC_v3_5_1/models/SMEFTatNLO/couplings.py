...
if configuration.bottomYukawa:
    ...
    GC_1842_bot = Coupling(name = 'GC_1842_bot',
                       value = '(c3pl1*complex(0,1)*vev0*ymb)/(2.*Lambda**2) + (c3pl2*complex(0,1)*vev0*ymb)/(2.*Lambda**2) - (cdp*complex(0,1)*vev0*ymb)/Lambda**2 - (cll1221*complex(0,1)*vev0*ymb)/(2.*Lambda**2) + (cpDC*complex(0,1)*vev0*ymb)/(4.*Lambda**2) + (cbp*complex(0,1)*vev0**2)/(Lambda**2*cmath.sqrt(2))',
                       order = {'NP':2,'QED':1})
